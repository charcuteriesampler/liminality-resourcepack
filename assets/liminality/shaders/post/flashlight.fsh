...
void main(){ 
    ...
    float FlashlightRadius = min((BlockDepth * 0.02) + 0.1, 0.3);
    float Brightness = max((FlashlightPower / BlockDistance) * ((FlashlightRadius - DistanceFromScreenCenter)/0.2) , AmbientBrightness);
